#!/usr/bin/env bash
# Publish data/analysis.json plus the camera stills to the orphan `data` branch
# as a single commit.
#
...
# rebuild on every one of the ~24 updates in a session - and it is the only way
# the stills render at all, since the LTA image host sends
# application/octet-stream with nosniff, which browsers refuse to draw.
set -euo pipefail
...
echo "  published $(ls -1 | wc -l) files to data branch"
